...
const arrowKeyIndexes = ["Up", "Left", "Down", "Right"].map((n) =>
	allKeys.indexOf(keyCodeToEle.get(`Arrow${n}`))
);
const macroKeys = [
	document.querySelector('[data-code="Escape"]'),
	...document.querySelectorAll("[data-macro]")
];
const furthestKeys = {};
	...
	for (const macro of macroKeys) {
		...
		for (let i = 0; i < allKeys.length; i++) {
			...
			const d = dist(
				...
				eleBounds.x + eleBounds.width * 0.5,
				eleBounds.y + eleBounds.height * 0.5
			);
			ele.macro = ele.macro || {};
			ele.macro[color] = d;
